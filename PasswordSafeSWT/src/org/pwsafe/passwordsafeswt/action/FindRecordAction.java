...
public class FindRecordAction extends Action {
    ...
    private class SearchState {
        ...
        /**
         * This updates the string that we're searching for and clears out the old results if it has actually changed.
	...
	public FindRecordAction() {
		...
		setImageDescriptor(ImageDescriptor.createFromURL(this.getClass().getClassLoader().getResource("org/pwsafe/passwordsafeswt/images/tool_newbar_find.png"))); //$NON-NLS-1$
...
//        app.getShell().getDisplay().addListener(SWT.KeyDown, (Listener)new FindKeyListener()); causes class cast ex
        ...
        // todo refactor to remove this overly-tight coupling? Comments or opinions gratefully received.
        findDialog.setCallingAction( this );
        findDialog.open();
	}

    /**
     * This figures out what to do with the search state and therefore with what the user wants from the search.
     * It also gives feedback to the user about the outcome.
     *
     * todo implement switching comparators
     * todo would be nice to search from the currently highlighted TreeItem rather than the top
     * todo reduce cyclic complexity of this
     *
     * @param newSearchString The search string that is currently in the dialog's text field.
     * @param previous Are we doing a find-previous?
     * @param equality Allows us to switch between a sub-string/case-insensitive/etc search
     */
    public void performSearch(final String newSearchString, final boolean previous, final FindMatcher equality) {
    ...
    private LinkedList<PwsEntryBean> findDataStoreEntries(final FindMatcher matcher) {
        ...
        PasswordSafeJFace app = PasswordSafeJFace.getApp();
        for(PwsEntryBean entry : app.getPwsDataStore().getSparseEntries() ) {
...
//        Collections.sort(  searchState.getResults(), PasswordSafeJFace.getApp().getSorter() );
    }
